...
To create a multi- scatter plot, you need to use the `groupBy` prop by passing a string into this prop. **This string should match the property name of your data that represents the distinct groups you want to show.** For example, in the above multi example dataset, our dataset is grouped by the property name 'fruit'.
...
### legend *(optional)*
Each chart has the option to include a legend which shows the categories and colors of each series of data. The legend and chart are both responsive, so they will resize to make space for each other.
